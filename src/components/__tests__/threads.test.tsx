...
import { Create } from "../threads/create/CreateThread";
...
describe("threadList", () => {
  ...
  it("navigate from root to thread creation page", async () => {
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route
            path="create"
            element={
              <Create threads={dataThreadMock} setThreads={setThreads} />
            }
          />
        </Routes>
      </BrowserRouter>
    ...
    await userEvent.click(
      screen.getByRole("link", { name: "Create New Thread" })
    );
  ...
  it("should be a new thread after create a thread", async () => {
    ...
    const setThreads = (
      newThreads: Thread[] | ((prevThreads: Thread[]) => Thread[])
    ) => {
    ...
    render(
      <BrowserRouter>
        <Routes>
          ...
          <Route
            path="create"
            element={<Create threads={threads} setThreads={setThreads} />}
          />
        </Routes>
      </BrowserRouter>
    );
  ...
  it("if a new thread is created, it should be in the thread list", async () => {
    let threads = dataThreadMock;
    const setThrads: React.Dispatch<React.SetStateAction<Thread[]>> = (
      newThreads
    ) => {
      threads =
        newThreads instanceof Function ? newThreads(threads) : newThreads;
    };
